...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/02/02 13:05:02 by recharif          #+#    #+#             */
/*   Updated: 2017/11/29 19:01:55 by recharif         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		char *base, t_arg *arg, char *prefix)
{
	...
	nbr_len = calc_nbrstrlen(nbr, base, NULL, arg) +
		ft_strlen(prefix);
...
ssize_t	floatprefix(double nbr, uintmax_t nbrint, t_arg *arg, char *prefix)
{
	size_t	nbr_len;
	size_t	nbr_cut;

	nbr_len = calc_nbrstrlen(nbrint, "0123456789", NULL, arg) +
		ft_strlen(prefix);
	nbr_cut = ft_strlen(prefix);
	if (arg->got_width && !arg->right_pad && !arg->pad_zeroes)
	{
		...
		arg->width -= ft_strlen(prefix);
	ft_putstr(prefix);
	return (printfloat(nbr - (double)nbrint, nbrint, arg, nbr_len));
}
